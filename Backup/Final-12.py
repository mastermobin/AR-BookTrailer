...
def getMatches(frameDes, desS, num=-1):
    matches = flann.knnMatch(queryDescriptors=frameDes,
...
def writeFrame(matrix, num, frame):
    ...
    result = ((pow(t1[:,:,0].astype('int32') - t2[:,:,0], 2) + pow(t1[:,:,1].astype('int32') - t2[:,:,1], 2) + pow(t1[:,:,2].astype('int32') - t2[:,:,2], 2)) < 9000).astype('uint8') * 255
    ...
    color = (255, 255, 255)
    cv.fillPoly(drawing, pts =[contours[max_id]], color=(255,255,255))
    # cv.drawContours(drawing, contours, max_id, color, 3)
...
while(cap.isOpened()):
    ...
    for num in range(10):
        ...
        qualified = getMatches(fdes, deses[num], num)
...
print("Duration: " + str(timeit.default_timer() - start))
